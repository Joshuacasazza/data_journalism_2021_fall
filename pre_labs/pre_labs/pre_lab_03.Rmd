...
**Answer** I saw 5 problem categories because the numbers in row and col were not in order. Under actual, there were dates and random sets of numbers. 1/0/T/F/TRUE/FALSE came up for every expected outcome. Nothing in file.
...
**Answer**I didn't see any errors. My data was broke up into 295865 rows and 63 columns. Then got 32 chr, 21 dbl, lgl 7, date 3.
...
**Answer**amount had <dbl> 149000.0, 146000.0, 140400.0, 13952~, name had <chr> "TARBIYAH ACADEMY INC", "J &AMP; M ~, id column had id numbers, ex <dbl> 99443772, 99444241, 99445051, 99445~
```{r}
...
**Answer**:192959 loans when NA. 275 loans when Subway.
...
**Answer**:135073, yes it is the same.
...
**Answer**0 (there was no october 2020)
```{r}
ppp_maryland_loans %>%
...
**Answer**10000000 largest and 6 smallest. I think 6 is very suspicious as that seems like a crazy low number. 10000000 is also suspicious for how high it is but I think that is a little more viable than 6. 
```{r}
...
**Answer**nothing showed up for me when I ran the code.
...
**Answer**<chr> "693361", "693361", "559000", "39700", "28422", "~
```{r}
...
**Answer**Inconsistency with the data like Stuart Jones coming up two times, Arnold being spelled wrong and capitalized wrong, addresses being wrote different
...
**Answer**added an x so its x1_id
...
**Answer**Error: Problem with `summarise()` column `total_amount`. i `total_amount = sum(amount)`. x invalid 'type' (character) of argument. Because it doesn't understand total_amount only amount.
...
**Answer**2 duplicate, STUART JONES LLC
