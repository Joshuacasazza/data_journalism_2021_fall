...
**Answer**51 rows of data with the 2016 voting results and the stats for business receiving and amount per employee.
```{r}
...
**Answer**plots business receiving ppe percent with the percent that voted for Trump.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer**Now its the percent voted for trump paired with ppe amount per employee.
```{r}
...
**Answer**The percent of both business and employee that voted for trump in 2016.
```{r}
reveal_data %>%
...
**Answer**Its Pearson's product-moment correlation with a 95% confidence interval and sample estimates along with t, df, and p-values.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**Same format but the values have changed because its for employees
```{r}
...
**Answer**The mean pp amount per employee and mean percent for business for trump and clinton voters.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**two sample t test that gets estimated means for both group trump and clinton.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer**Same as above but now mean percent for businesses.
```{r}
