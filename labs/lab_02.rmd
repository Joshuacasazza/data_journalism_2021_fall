...
```{r}
  west_virginia_with_group_loans <- west_virginia_ppp %>%
    mutate( 
      group_loans = case_when(
        amount < 10000 ~ "under_10k",
        amount > 50000 & amount < 100000 ~ "10k_50k",
...
```
```{r}
  west_virginia_with_group_loans %>% group_by(group_loans) %>%
  summarise(total = n()) %>%
  arrange(desc(total))
...
**A2** Harvest Small Business Finance, LLC with 139 total
...
**A3** A lot are banks who have given more than 14 loans

**Q4** Which month in 2021 had the most loans approved?
**A4** March had the most approved with 4404
