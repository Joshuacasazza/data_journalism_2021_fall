...
justice_html <- justice_url %>%
  read_html()

justice_html %>% html_elements("ul b") %>%
...
cases <- justice_html %>% html_elements("b i") %>%
...
A4. Smith is a recording artist form Florida who was accused of filing fraud loan applications for millions of dollars. If I were an editor I would have assigned someone to cover this case since it was two men, one from FL and the other from PA, getting millions of dollars and spending it on a bunch of stuff like cars. The fact that an NFL player was involved makes this story that much crazier and readable. 
...
new_wiki <- wiki_code[[3]]


new_wiki %>%
  summarize(pct=nrow(cases)/nrow(new_wiki))
```
...
A6.Certain judicial districts may have significantly less people and businesses and there for would have less fraud. 
...
A7.Sorry I really couldn't figure this code out. I'm completely lost on where to go next I tried everything I thought could work from the pre_lab. 
```{r}
states_url <- "https://smussenden.github.io/coursefiles/ppp_scraping_example/index.html"

states_table <- states_url %>%
  read_html() %>%
  html_table() 

state_data <-states_table[[1]]
  
for (row_number in 1:nrow(state_data)) {
  print(row_number)

each_row_df <- state_data %>%
      slice(row_number)
