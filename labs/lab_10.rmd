...
barber_app_state <- barber_ppp %>%
    group_by(state) %>%
    summarise(count=n()) %>%
    arrange(desc(count))

barber_app_state_loan_zero <- barber_ppp %>%
    filter(undisbursed_amount > 0) %>%
    group_by(state) %>%
    summarise(over_zero=n()) %>%
    arrange(desc(over_zero))

joined_data <- barber_app_state_loan_zero %>% left_join(barber_app_state, by=c('state')) %>% top_n(10)

write_csv(joined_data, 'joined_data.csv')
```

**A1.**https://datawrapper.dwcdn.net/KFzGf/1/ 
Illinois has the most of any state with 11080 applications. The amount of undisbursed in Illinois is 1713. They also have the highest number of undisbursed. 
...
joined_data <- barber_app_state_loan_zero %>% left_join(barber_app_state, by=c('state'))

percent_joined <- joined_data %>%
  mutate(percent = (over_zero/count)*100) %>%
  arrange(desc(percent))

write_csv(percent_joined, 'percent_joined.csv')
```

**A2.**https://datawrapper.dwcdn.net/h976w/1/
...
joined_data_two <- kabbage_counties %>% left_join(all_counties, by=c('project_county_name', 'project_state')) 

kabbage_joined <- joined_data_two %>%
  inner_join(all_counties, by=c('project_county_name', 'project_state')) %>%
  rename(kabbage=total.x, all_total=total.y) %>%
  mutate(percent = (kabbage/all_total)*100) %>%
  arrange(desc(percent))

county_fips <- read_csv("data/county_fips.csv") %>%
  rename(project_county_name=match_county, project_state=state)

kabbage_fips <- kabbage_joined %>% left_join(county_fips, by=c('project_county_name', 'project_state')) 

write_csv(kabbage_fips, 'kabbage_fips.csv')
```
**A3.**https://datawrapper.dwcdn.net/h8h8K/1/
They seem to be the most common on the east coast, specifically the south east with the majority being in FL. Yes there are counties with no data. I believe that is because there were no loans by Kabbage so when we divided it was 0/the loans which is 0%.
