...
A1: Charleston (2837 loans), South Charleston	(484 loans), Saint Albans	(304 loans), Dunbar (166 loans), Nitro	(142 loans)
```{r}
# cleaning function
...
wv_ppp_loans %>%
 filter(project_county_name == "KANAWHA") %>%
  mutate(project_city = str_to_title(city)) %>%
  mutate(project_city = case_when(
    project_city == "Charelston" ~ "Charleston",
    str_detect(project_city,"^Charelston") ~ "Charleston",
    project_city == "S Charleston" ~ "South Charleston",
    project_city == "So Charleston" ~ "South Charleston",
    project_city == "S Charleston" ~ "Saint Albans",
    project_city == "St. Albans" ~ "Saint Albans",
    project_city == "St Albans" ~ "Saint Albans",
    project_city == "St.albans" ~ "Saint Albans",
    project_city == "Crosslanes" ~ "Cross Lanes",
    project_city == "Elkveiw" ~ "Elkview",
    project_city == "Balle" ~ "Belle",
    TRUE ~ project_city
  )) %>%
...
A2: JACKSON KELLY PLLC had the highest total loan in Charleston. The loan was for 5068472.00. 5126618.64 of the loan was forgiven. The forgiveness amount was large than the loan amount. Loans accrue interest for PPP loans.
...
A3: The names of the five businesses are DM MOTOR, INC, DUTCH MILLER CHEVROLET, INC., DUTCH MILLER OF CHARLESTON, INC, DUTCH MILLER OF CHARLOTTE, INC and DUTCH MILLER SUBARU, INC. They are car dealerships in West Virginia and North Carolina. In total, the five business combine for $3,105,300. Over 100% of the loans were forgiven as the total was $3,140,471. Miller voted "yea" to the program. 
